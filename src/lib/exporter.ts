...
// CORRECTION ICI: La fonction accepte maintenant SiteConfig au lieu de any
function getNavbarHtml(config: SiteConfig, activePage: string = 'home') {
  const meta = config.meta; // On extrait meta pour les utilisations ci-dessous
  const brandColor = meta?.theme?.brandColor || meta?.theme?.primaryColor || "#F97316";
  // CORRECTION ICI: On passe l'objet 'config' complet
...
function generateHomePage(config: SiteConfig): string {
  ...
  // CORRECTION APPEL: On passe 'config' au lieu de 'meta'
