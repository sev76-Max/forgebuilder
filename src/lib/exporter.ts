...
function getNavbarHtml(meta: any, activePage: string = 'home') {
  const brandColor = meta?.theme?.brandColor || meta?.theme?.primaryColor || "#F97316";
  const logoHtml = `<a href="index.html" style="text-decoration: none; display: flex; align-items: center;">${generateLogoHtml({ meta })}</a>`;
